...
// project_seed.json — 주소 1회 해석 → 세 앱이 각자 채움 (schema_version 'project_seed/1.0')
{
  ...
  "model":    { /* arch-site-model: 물리 3D(건물·터레인·지적 geometry + origin_offset·.3dm/.skp) */ },
  "base_date": "..."
}
```
...
- **✅ arch-site-model 결합 완료 (2026-07-09)**: 물리 3D + 인문 = 완전한 보드. `ProjectSeed.model` 슬롯(law·knowledge 대칭, arch-site-model 소유 느슨 dict). **방식=계약 확장 + 받은 모델 렌더 — 터읽기는 arch-site-model 을 호출하지 않는다(provider 경계)**: assembler(competition·kw-ai-hub·Claude)가 arch-site-model `POST /api/generate` 출력을 `POST /board {model:...}` 로 넘기면 `services/site_model.summarize_model`이 압축 요약(building_count·elev_range·**origin_offset**·footprints≤400·files) → `BoardResult.model` + `/board/view` **축측 매싱 미리보기**(three.js 없이 서버사이드 PIL, 자체완결). **정합성**: 두 앱 같은 주소·**같은 PNU**(VWorld `LP_PA_CBND_BUBUN`)·같은 VWorld → site 공유만으로 결합. `origin_offset`(EPSG:5186 씬 원점) 보존해 좌표 복원 가능.
