...
| **KOSIS OpenAPI** | 시군구/읍면동 인구·가구 통계 (연령 5지표 + 1인가구비율 등, census 다차원) | 캐시 우선, 분당 제한. 읍면동 세대수는 미제공 |
| **행안부 주민등록 (rdoa)** | 행정동별 인구 + **세대수** (KOSIS 미제공 — 조사범위 통계용) | 무키·전국·월별, HTML 파싱 |
...
│   │   ├── jumin.py         #   행안부 rdoa 행정동별 인구+세대 (무키·전국, 심의 현황팩)
