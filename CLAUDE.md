...
| **종합·해석** | S1 | 데이터 근접도 레이어 (모든 수치에 대지근접도 등급: 대지>반경>읍면동>시군구>proxy) | 📋 계획 (§8.11) |
| | S2 | 교차규칙 엔진 (도메인 횡단 '참고' 시사점 + 근거, JSON·LLM 0) | 📋 계획 (§8.11) |
| | S3 | `/board` 통합 진입점 (전 도메인 + S2 + 결측목록, 기존 서비스 재사용·병렬) | 📋 계획 (§8.11) |
| | S4 | 종합 산출 = 해석(Sonnet, 그라운디드) + 판단(Opus, 분리·라벨) 두 블록 | 📋 계획 (§8.11) |
...
- **S1 데이터 근접도 레이어** (기반·작음) — 모든 fact/signal에 `proximity` 등급 통일: `대지(필지) > 반경 > 읍면동 > 시군구 > proxy/추정`. 기존 scope_level을 한 축으로 정규화. "구 평균은 대지값 아님" 원칙이 모든 출력에 기계가독·정렬가능하게 박힘. 순수 메타데이터 → 환각 0.
- **S2 교차규칙 엔진** (해석의 뼈대) — `cross_context.json` 신설. implications.json 패턴을 **도메인 횡단**으로 확장. 통합 fact 풀(인구+수급 signal+재해)을 읽어 근거(basis) 달린 '참고' 시사점 생성. 예: 고령↑+의료수급 적음→"의료 접근성 검토" / 홍수영향+지하건물↑→"지하 침수 대비 검토" / 1인가구↑+폭염 빈발→"취약계층 폭염 대응 검토". **전부 규칙 매칭, LLM 0, 새 숫자 안 만듦**(기존 fact 값 boolean 조합).
- **S3 `/board` 통합 진입점** — 기존 서비스(analyze·diagnose·site·seed) **재사용·병렬 오케스트레이션**(새 데이터 0). 한 객체: 공유 site + 도메인별 facts(근접도 부착) + 수급진단 + 재해 + S2 교차시사점 + **결측/확인불가 목록**(no silent skip). 프론트 종합 탭.
- **S4 종합 산출 = 두 블록 (벽 분리·라벨)** ★ — S1~S3 구조물 위에서:
  - **① 사실 종합 (해석)** — 그라운디드. 실수치만·모든 문장 fact 추적·근접도/출처 명시·'참고'. "이 필지는 ~한 곳" 특징서술. **모델 Sonnet 4.6**(서술, 싸고 충분). P6 narrative 패턴.
  - **② AI 판단 (의견)** — 명확히 분리·라벨("아래는 AI 의견·검증/재현 보장 없음·최종결정은 사람"). 용도 관점 적합신호(±)·종합 의견. **조건: (a) 근거 fact 인용 (b) 가정 명시 (c) 새 숫자 안 만듦**. **모델 Opus 4.8**(추론 정교).
...
이 설계는 **§2.5(판단은 사람) → "판단은 분리·라벨해 제시하되 *최종 결정*은 사람"으로 완화**한다. S4 구현 시 §2.5 문구를 갱신할 것(의도된 정체성 진화). S1~S3 단계는 이 완화와 무관(사실·메타·규칙뿐).
