...
**diagnose 연결** — `DiagnoseRequest.resolution="반경"`(Resolution에 추가). demand 3지표(유소년·고령·생산가능인구비율)를 SGIS 반경값으로 교체(national_avg는 KOSIS 전국값 유지·비교 일관). **1인가구비율은 SGIS 집계구 미제공 → 시군구 폴백+note.** SGIS 실패 시 전체 시군구 폴백(graceful). region.resolution="반경", name="○○구 반경 Nm". 프론트 C탭 "반경(집계구)" 토글.

**검증(여의대로24)**: 500m=집계구7개·3,635명. 1km=62개(60매칭)·29,281명·유소년5.6%·고령16.1%·생산78.2%(1km는 금융지구 포함→동단위 11%보다 유소년 낮음=반경 정밀도). 미매칭 집계구(한강·공원 무인구)는 note. 테스트: 비라이브 2(monkeypatch)+라이브 2(`test_sgis_live.py`). `verify_apis.py probe_sgis` 추가.
