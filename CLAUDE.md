...
- 배포: GCP Cloud Run + Secret Manager(키). 캐시는 **파일캐시**(`services/cache.py FileCache`, `OUT_DIR=/tmp/out` — Cloud Run 임시 FS라 인스턴스별·비영속). GCS 캐시(`GCSCache`)는 `GCS_CACHE_BUCKET` 설정 시 활성(**프로덕션 연결됨** — 생성 이력·KOSIS 캐시 GCS 영구저장), 미설정 시 파일캐시 폴백.
...
**★의료인력 공급측 보강 (2026-07-15, KOSIS Phase3 연계)** — 의료시설 수급에 시군구 **의료인력**(HIRA `DT_HIRA4U`, census 크랙) 편입. 정원과 동일 패턴: 규칙 `workforce_source:"medical"` → `_collect_workforce`가 `census_multidim.fetch_census_indicator`로 시군구 의료인력 조회 → `cross_rules(..., workforce_data=)` → `SupplySignal.workforce`·`workforce_per_10k`(시군구 총인구 만명당)·`workforce_scope`. **반경 시설개수(판정)만으론 대형병원 인력규모를 못 봄** → 실제 의료 공급 캐파 보강(단위 다름·시군구·참고, 절대 원칙 4). graceful. 검증 영등포: 반경 병원·의원·약국 352개 + 시군구 의료인력 8,194명·220.6/만명. 프론트 C탭 노출.
...
## 8.14 컨셉·설계방향 제안 커버 (발표용) ✅ 완료 (2026-07-16) — S4 원칙의 *의도적·격리된* 예외

종합읽기 PPT 마지막에 붙는 "PROJECT DIRECTION" 커버 한 장. 레퍼런스 이미지(컨셉명 + 키워드 3개로 설계 방향 제안)처럼, 지금까지의 분석을 **설계 컨셉 방향**으로 번역해 발표 임팩트를 준다. 이건 **창작(네이밍·컨셉)** 이라 S4 ②의 "새 숫자 금지"를 넘어 "새 서사"까지 만든다 — 앱의 다른 곳에선 금지된 행위다.

**왜 허용하나 (사용자 결정)**: 발표용 임팩트를 위해 *이 한 기능에 한해* 원칙 완화. 대신 **격리(quarantine)** 로 나머지 앱의 정직성을 지킨다:
1. **벽 + 라벨 유지** — 코드가 항상 `CONCEPT_LABEL`("AI 제안·검증/재현 보장 없음·최종 컨셉·제안서는 사람") 부착. 모델에 안 맡김. 슬라이드도 amber 칩·핑크 강조로 '① 검증된 사실' 슬라이드와 시각 분리.
2. **근거 인용 유지** — 컨셉명은 창작이되, 각 키워드는 어느 드라이버/사실에서 나왔는지 `basis` 로 추적. 근거 없는 키워드 금지.
3. **★ 기계 계약에 절대 안 흘림** — 결과는 plain dict(스키마 없음), **`board_pptx` 경로에서만** 계산해 덱 dict 에 주입. `BoardResult`·`board_brief`·`project_seed`·MCP 어디에도 안 들어감 → competition 형제앱이 우리 컨셉을 실측인 척 삼켜 이중 의견 나는 것을 원천 차단. 실제 컨셉·네이밍은 여전히 competition 몫(경계 유지).

**구현**: `services/concept.py`(`derive_concept` → Opus `claude-opus-4-8`, synthesis ②와 동일. `_pool_text`/`_call` 재사용. JSON 출력 파싱·검증). 근거(드라이버/사실) 없으면 None(억지 창작 금지). 키 없음·오류·파싱 실패 → **규칙 폴백**(드라이버 이름을 키워드로, 창작 네이밍 없음 — 정직). `board_slides.slide_concept`(`board['concept']` 있을 때만 렌더). `BoardRequest.concept:bool=False`(opt-in). 프론트 I탭 체크박스("컨셉·설계방향 제안 커버 — PPT 전용·AI 창작·계약 미포함"). test_concept 8(폴백·격리·파싱·라벨).

---

## 8.15 덱 다음 후보 — kunwon-ops 4단계 계획 (2026-08-25, 미착수)

`kunwon-ops`(사내 앱 운영 판단 저장소, private) 가 상용 벤치마킹으로 지목하고 대상 앱을
재확인한 두 항목. 근거·비교 대상은 `kunwon-ops` 저장소
...
- **사업체수·종사자수** [확정조사 완료 → 보류]: KOSIS에 시군구 단위 전체산업 사업체수가 깔끔히 없음.
  ...
  - ★ 더 근본: raw 사업체수를 전국 총량과 비교(현 national_avg 모델)는 무의미. **사업체밀도(개/㎢)·인구당 사업체수**로 재설계해야 의미. → **✅ 인구당 정규화로 해소 (2026-07-15, 모드A 밀도정규화)**: `services/census_density.py`+`data/census_density.json`. 시군구 census/시군구인구 vs **전국 census/전국인구**(둘 다 per-천명, 전국값은 크랙 엔진이 region '전국' 멤버로 **동적 산출**·하드코딩 0) → 지수 자동. `/analyze density=true`(opt-in, census 호출로 느려서), 프론트 A탭 "census 밀도지표" 체크박스. 검증 영등포: 사업체 261.2/천명·지수177(상업중심)·빈집 6.2·지수20(수요탄탄)·신혼부부 지수131. 밀도(개/㎢)는 면적분모(VWorld) 확보 후 후속.
...
- **인프라 부채** — StarletteDeprecation httpx 경고(D7), GCS 캐시 ✅ **연결 완료**(GCS_CACHE_BUCKET·생성이력·KOSIS) — 잔존은 TTL 자동갱신(현재 무한 보관)뿐(D8), 합성 PNG 다중인스턴스(D10).
