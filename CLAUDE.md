...
| POST | `/board` | S3·S4 | 대지 종합 읽기 — analyze·diagnose·site·seed **병렬 오케스트레이션**(새 데이터 0) + S1 근접도 + **S2 교차시사점**(cross_context) + 도메인 coverage(결측 투명). `synthesize=true`면 **S4 종합 산출**(①사실 해석=Sonnet·②AI 판단=Opus, 벽 분리·라벨, `services/synthesis.py`). 종합점수 없음. 프론트 I탭 |
| GET | `/health` | - | 헬스체크 |
...
| | T1.5 | 대지 아키타입(동네 유형) — 규칙 룩업·한글 유형명·2계층 (Esri Tapestry, K-means 아님) | 📋 계획 (§8.12) |
| | T2 | 설계 드라이버 합성 (분석→설계 다리, 지배 드라이버 2~3개) ★blue ocean | 📋 계획 (§8.12) |
| | T3 | 프로그램 함의(POR seeds) — 드라이버→공간·프로그램 권고 (program_rules.json) ★blue ocean | 📋 계획 (§8.12) |
| | T4 | 대지분석 보드 + 딜리버리 (공유링크·PDF·API, 지도앵커·패널그리드) | 📋 계획 (§8.12) |
| | T5 | 방법론·데이터 부록 (공공 공모·감사 대비) | 📋 계획 (§8.12) |
...
- **T1.5 대지 아키타입** 📋 — Esri Tapestry식 "이 동네는 ○○형"(고령 정주형·1인가구 도심임대·육아 신도시형·재건축 대기 노후주거 등). ⚠️ K-means(통계·해석) 금지 → **규칙 룩업**(`archetype.json`, implications 패턴)·2계층·근거 동봉·persona 문단만 S4식 AI표현. /board 풀·cross_context 엔진 재사용.
- **T2 설계 드라이버 합성** 📋 ★ — 교차시사점+facts를 증거강도로 랭킹 → 지배 드라이버 2~3개("분석의 종착점=design driver", 리서치). **상용 아무도 없음.**
- **T3 프로그램 함의(POR)** 📋 ★ — 드라이버→공간·프로그램 권고(평면/코어/공용부/저층부/방재), `program_rules.json`. **상용 아무도 없음.**
- **T4 대지분석 보드 + 딜리버리** 📋 — 지도앵커+핵심수치 타일 그리드+지수막대+벤치마크행(반경/동/구/전국)+아키타입+드라이버+출처푸터 → 공유링크(Artifact)·PDF·API(/board). Esri 패널그리드·Local Logic 웹링크우선.
- **T5 방법론·데이터 부록** 📋 — 사용 통계표·API·산정식 자동 부록(Esri 방법론공개·토지이음 확인원 방식).

**추천 순서:** T1(완료) → T2(설계 다리) → T4(임원용 공유 보드) → T1.5·T3·T5. **금지선 유지:** AI 렌더·자동 매싱·수익률 추정 안 만듦(랜드북·Forma 레드오션·환각 위험). 우리는 "지어내지 않는 근거 + 설계 번역 + 정직한 포장".
