...
**규칙 확장 (2026-06-29)** — supply_demand.json에 **의료시설 수급**(고령인구비율×병원·의원·약국, low_max=10, high_min=30)·**초등학교 수급**(유소년인구비율×초등학교, low_max=0, high_min=3) 추가 → 총 5개 규칙. 코드 변경 없음, JSON만으로 자동 반영(절대 원칙 7). 문화시설은 단일 수요 proxy 없음 → /seed context에만(현황 제공, 수급진단 미편입).
