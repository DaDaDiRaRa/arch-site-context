...
**남은 것:** C2 총량제(`community_quota.json` — 세대수×1.5×4%·소요면적·부족/충족)·C3 어린이집 형태(cpmsapi030 운영계정 전환 대기)·C4 조사범위 위치도 PNG(경로당·어린이집 증명됨)·C5 편집가능 PPTX(슬라이드 추출+표 채움 증명됨)·C6 `/survey`·`/context-pack` 오케스트레이션. **C1·jumin 외는 아직 scratchpad(증명 완료·배선 전).**
