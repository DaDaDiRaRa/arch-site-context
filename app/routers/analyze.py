...
한 문단 서술(narrative)은 P6 — 지금은 규칙 기반 초안 + rule_based_fallback.
"""
...
def analyze(req: AnalyzeRequest):
    ...
    # 3) 함의 룩업 (규칙 기반, LLM 없음 — 절대 원칙 2)
    imps = derive_implications(facts, use_type=req.use_type)

    # 4) 한 문단 서술 (Claude 1회, 실패 시 규칙 폴백). '○○구 기준'·연도 명시 (절대 원칙 4)
    ...
    region_stat = RegionStat(
        ...
        source=source,  # 'ai' 또는 'rule_based_fallback'
        notes=notes,
    )
    return region_stat
