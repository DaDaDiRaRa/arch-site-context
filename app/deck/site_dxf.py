...
#: 참조원 반경(m) — model 요청 반경(350m 고정, clients.fetch_model)에 맞춰 그 안쪽만
_REF_RADII = (100, 200, 350)


def _parcel_ring(geom):
    """law.parcel_geometry(MultiPolygon/Polygon, WGS84 [lon,lat]) → 최대 외곽링 [(lon,lat),...]."""
    if not geom:
        return []
...
def build_site_dxf(address: str, use_type: str = "주거", radius: int = 1000) -> bytes:
    ...
    with ThreadPoolExecutor(max_workers=3) as ex:
        ...
        f_fac = ex.submit(clients.fetch_facilities, address, list(ms.KIND_USE.keys()), 320)
    ...
    ring = _parcel_ring((law or {}).get("parcel_geometry"))
